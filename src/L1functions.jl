...
function _L1_drift!(dZ, Z, (true_system, L1params, Λhat, iₐ), t; kwargs...)
    ...
    # Need the following unrefined concatenation for StaticArrays and GPU compatibility
    X = Z[1:n] # System state
    Xhat = Z[n+1:2n] # Predictor state
    Xfilter = Z[2n+1:2n+m] # Low pass filter state
    ##########################
    ### CONTROLLER 
    ##########################
    # - L1 input 
    gbar(t) = hcat(g(t), g_perp(t)) # Full rank
    Θ(t) = hcat(I(m), zeros(m,n-m))*inv(gbar(t))   
    Λhat_m = Θ(iₐ*Tₛ)*Λhat # Matched uncertainty estimate 
    m == 1 ? uₐ=-only(Xfilter) : uₐ=-Xfilter    
...
function _L1_diffusion!(dZ, Z, (true_system, L1params), t; kwargs...)
    ...
    dXfilter = zeros(m,d) # Filter is an ODE (drift only)
    concat_diffusion = vcat(dX, dXhat, dXfilter)
    for i in 1:2n+m
		for j in 1:d
            dZ[i,j] = concat_diffusion[i,j]
		end
...
function system_simulation(simulation_parameters::SimParams, true_system::TrueSystem, L1params::L1DRACParams; kwargs...)
    ...
    L1_init = vcat(true_init, true_init, zeros(m)) # System and predictor initialized by the same initial condition, and filter state initialized at zero
    Λhat = zeros(n) # Adaptive estimate over the first interval [0, Tₛ)
    iₐ = 0.0 # index for the piecewise interval
    #Define the problem
	L1_problem = SDEProblem(_L1_drift!, _L1_diffusion!, L1_init, tspan, noise_rate_prototype = zeros(2n+m, d), (true_system, L1params, Λhat, iₐ))
    #############################################################
    ...
    adaptive_law_callback = DiscreteCallback(_adaptive_law_condition, _adaptive_law_affect!, save_positions=(false, false))
    #############################################################
	# Solve the problem
