...
function plotfunc(filename, tspan, ylimits)
    l = @layout [a b; c d]
    ...
    for i ∈ 1:Ntraj
        plot!(p1, ens_nom_sol[i], idxs = (0,1), lw=2, lalpha=0.1, color = 13, label = :false, xlims=tspan, ylims=ylimits)
        plot!(p1, ens_tru_sol[i], idxs = (0,1), lw=2, lalpha=0.1, color = 7, label = :false, xlims=tspan, ylims=ylimits)
        plot!(p1, ens_L1_sol[i], idxs = (0,1), lw=2, lalpha=0.1, color = 25, label = :false, xlims=tspan, ylims=ylimits, title = L"X_1-Nom(Blue), Tru(Orange) \mathcal{L}_1(Green)")

        plot!(p2, ens_nom_sol[i], idxs = (0,2), lw=2, lalpha=0.1, color = 13, label = :false, xlims=tspan, ylims=ylimits)
        plot!(p2, ens_tru_sol[i], idxs = (0,2), lw=2, lalpha=0.1, color = 7, label = :false, xlims=tspan, ylims=ylimits)
        plot!(p2, ens_L1_sol[i], idxs = (0,2), lw=2, lalpha=0.1, color = 25, label = :false,xlims=tspan,ylims=ylimits ,title = L"X_2")
    end
    plot!(p3, nom_summ, idxs = 1, color = 13, lw = 1, fillalpha = 0.2, xlims=tspan,ylims= ylimits)
    plot!(p3, tru_summ, idxs = 1, color = 7, lw = 1, fillalpha = 0.2, xlims=tspan,ylims= ylimits)
    plot!(p3, L1_summ, idxs = 1, color = 25, lw = 1, fillalpha = 0.2, xlabel = "t", xlims=tspan, ylims=ylimits)

    plot!(p4, nom_summ, idxs = 2, color = 13, lw = 1, fillalpha = 0.2, xlims=tspan,ylims= ylimits)
    plot!(p4, tru_summ, idxs = 2, color = 7, lw = 1, fillalpha = 0.2, xlims=tspan,ylims= ylimits)
    plot!(p4, L1_summ, idxs = 2, color = 25, lw = 1, fillalpha = 0.2, xlabel= "t", xlims=tspan, ylims=ylimits)
