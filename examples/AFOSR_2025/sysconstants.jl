...
function _DeltaRHat(assumption_constants::AssumptionConstants)
    ...
    @unpack p, Lipschitz_condn_holds,  Δg, Δg_dot, Δf, Δσ, Δp, Δμ, Δ_star, L_f, λ = assumption_constants
...
function _DeltaR_circle(assumption_constants::AssumptionConstants, ΔrHat::DeltaRHat)
    @unpack p,  Δp, Δσ, Δμ_parallel, Δσ_parallel, Δp_parallel, Δg, λ, m = assumption_constants
    @unpack ΔrHat_1 = ΔrHat

    Δr_circle_1 = Δp^2 + (Δp + Δσ)^2
    Δr_circle_2 = (Δμ_parallel / sqrt(λ)) * (ΔrHat_1 + (Δg^2 * Δμ_parallel) / sqrt(λ))
    Δr_circle_3  = (𝔭_prime(p) / sqrt(λ)) * (Δp_parallel + Δσ_parallel) * (ΔrHat_1  + (2 * Δg^2 * Δμ_parallel) / sqrt(λ))
    Δr_circle_4 = (Δp_parallel + Δσ_parallel) * Δg / λ *((𝔭_prime(p)^2) * Δg * (Δp_parallel + Δσ_parallel)) + sqrt(m) * (2*Δp + Δσ)
...
function _DeltaR_odot(assumption_constants::AssumptionConstants, ΔrHat::DeltaRHat)
    @unpack p,  Δg, Δg_perp, Δσ, Δσ_parallel, Δσ_perp, Δp, Δμ, Δμ_parallel, Δμ_perp, Δp_parallel,Δp_perp,  λ, m = assumption_constants
    ...
    Δr_odot_6 = (𝔭_prime(p) / sqrt(λ)) *
                ((Δp_parallel + Δσ_parallel) * (ΔrHat_3+ (2 * Δg^2 * Δμ_parallel) / sqrt(λ)) +
                Δσ_parallel * ΔrHat_2)
    Δr_odot_7 = 𝔭_prime(p) * (Δp_parallel + Δσ_parallel) * (ΔrHat_4 / sqrt(λ) + 2 * Δg)
    Δr_odot_8 = Δσ_parallel * (Δg / λ) *
                ((𝔭_prime(p)^2) * Δg * Δσ_parallel + sqrt(m) * Δσ)
...
function _DeltaR_otimes(assumption_constants::AssumptionConstants, ΔrHat::DeltaRHat)
    @unpack p,  Δg, Δg_perp, Δσ, Δσ_parallel,Δσ_perp, Δp, Δμ_parallel, λ = assumption_constants
    ...
    Δr_otimes_4 = 𝔭_prime(p) * Δσ_parallel *
                  ((ΔrHat_3 + (2 * Δg^2 * Δμ_parallel) / sqrt(λ)) / sqrt(λ))
    Δr_otimes_5 = 𝔭_prime(p) * Δσ_parallel * (ΔrHat_4 / sqrt(λ) + 2 * Δg)
...
function _DeltaHat(assumption_constants::AssumptionConstants)
    @unpack p, Lipschitz_condn_holds, Δg, Δf, L_p, L_σ, L_μ, L_f, Δg_dot, λ, m  = assumption_constants
    
    ΔHat_1= (2 / sqrt(λ)) * Δg * Δf * (1 - I_Lip(Lipschitz_condn_holds))
    ΔHat_2 = Δg * 𝔭(p) * (L_p + L_σ)
...
function _Delta_circledcirc(assumption_constants::AssumptionConstants, ΔHat::DeltaHat)
    @unpack p, L_p_parallel, L_σ_parallel, λ  = assumption_constants
    @unpack ΔHat_1   = ΔHat
    Δ_circledcirc_1 = (1 / sqrt(λ)) * 𝔭_prime(p) * (L_p_parallel + L_σ_parallel) * ΔHat_1  
    return Delta_circledcirc(Δ_circledcirc_1)
end

function _Delta_odot(assumption_constants::AssumptionConstants, ΔHat::DeltaHat)
    @unpack p, L_p, L_σ, L_p_parallel, L_σ_parallel, L_μ_parallel, λ, Δg = assumption_constants
    @unpack ΔHat_1, ΔHat_2, ΔHat_5   = ΔHat
   
    Δ_odot_1 = (L_p + L_σ)^2
    Δ_odot_2 = (1 / sqrt(λ)) * L_μ_parallel * ΔHat_1
    Δ_odot_3 = (1 / sqrt(λ)) * 𝔭_prime(p) * (L_p_parallel + L_σ_parallel) * ΔHat_2
    Δ_odot_4 = (1 / λ) * (L_p_parallel + L_σ_parallel) * ( ΔHat_5  +
               Δg^2 * (𝔭_prime(p)^2) * (L_p_parallel + L_σ_parallel) )
...
function _Delta_otimes(assumption_constants::AssumptionConstants, ΔHat::DeltaHat)
    @unpack p, Δg, Δg_perp, L_p_parallel, L_σ_parallel, L_p_perp, L_σ_perp, L_μ_parallel, λ = assumption_constants
    @unpack ΔHat_2, ΔHat_3, ΔHat_4   = ΔHat

    Δ_otimes_1 = 2 * Δg_perp * 𝔭(p) * (L_p_perp + L_σ_perp)
    Δ_otimes_2 = 2 * sqrt(λ) * Δg * 𝔭(p) * (L_p_parallel + L_σ_parallel) +
                L_μ_parallel * ΔHat_2 / sqrt(λ)
    Δ_otimes_3 = (1 / sqrt(λ)) * 𝔭_prime(p) * (L_p_parallel + L_σ_parallel) * ΔHat_3
    Δ_otimes_4 = 𝔭_prime(p) * (L_p_parallel + L_σ_parallel) *
...
function _Delta_ostar(assumption_constants::AssumptionConstants, ΔHat::DeltaHat)
    @unpack p,  Δg, Δg_perp, L_μ_perp, L_μ_parallel, λ = assumption_constants
