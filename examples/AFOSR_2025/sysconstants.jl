...
function DeltaRHat_expr(assumption_constants::AssumptionConstants)
...
function DeltaR_circle_expr(assumption_constants::AssumptionConstants, ΔrHat::DeltaRHat)
...
function DeltaR_circledcirc_expr(assumption_constants::AssumptionConstants, ΔrHat::DeltaRHat)
    ...
    Δr_circledcirc_3 = (𝔭_prime(order_p) / sqrt(λ)) *
                       ((Δp_parallel + Δσ_parallel) * ΔrHat_2 + Δσ_parallel * (ΔrHat_1+ (2 * Δg^2 * Δμ_parallel) / sqrt(λ)))
...
function DeltaR_odot_expr(assumption_constants::AssumptionConstants, ΔrHat::DeltaRHat)
    ...
    Δr_odot_6 = (𝔭_prime(order_p) / sqrt(λ)) *
                ((Δp_parallel + Δσ_parallel) * (ΔrHat_3+ (2 * Δg^2 * Δμ_parallel) / sqrt(λ)) + Δσ_parallel * ΔrHat_2)
...
function DeltaR_otimes_expr(assumption_constants::AssumptionConstants, ΔrHat::DeltaRHat)
    ...
    Δr_otimes_4 = (𝔭_prime(order_p) * Δσ_parallel / sqrt(λ)) * (ΔrHat_3 + (2 * Δg^2 * Δμ_parallel) / sqrt(λ))
...
function DeltaR_ostar_expr(assumption_constants::AssumptionConstants, ΔrHat::DeltaRHat)
    @unpack Δg,Δg_perp, Δμ_parallel,Δμ_perp, λ = assumption_constants
    @unpack ΔrHat_3, ΔrHat_4   = ΔrHat

    Δr_ostar_1 = 2 * Δg_perp * Δμ_perp
    Δr_ostar_2 = (Δμ_parallel/ sqrt(λ)) * (ΔrHat_3 + (Δg^2 * Δμ_parallel) / sqrt(λ))
    Δr_ostar_3 = Δμ_parallel * (ΔrHat_4  / sqrt(λ) + 4 * Δg)
    return DeltaR_ostar(Δr_ostar_1, Δr_ostar_2, Δr_ostar_3)
end


ref_sys_constants(assump_consts::AssumptionConstants) = begin
    DeltaRHat = DeltaRHat_expr(assump_consts)
    DeltaR_circle = DeltaR_circle_expr(assump_consts,  DeltaRHat)
    DeltaR_circledcirc= DeltaR_circledcirc_expr(assump_consts,  DeltaRHat)
    DeltaR_odot= DeltaR_odot_expr(assump_consts,  DeltaRHat)
    DeltaR_otimes = DeltaR_otimes_expr(assump_consts,  DeltaRHat)
    DeltaR_ostar = DeltaR_ostar_expr(assump_consts,  DeltaRHat)
...
function DeltaHat_expr(assumption_constants::AssumptionConstants)
...
function Delta_circledcirc_expr(assumption_constants::AssumptionConstants, ΔHat::DeltaHat)
...
function Delta_odot_expr(assumption_constants::AssumptionConstants, ΔHat::DeltaHat)
...
function Delta_otimes_expr(assumption_constants::AssumptionConstants, ΔHat::DeltaHat)
...
function Delta_ostar_expr(assumption_constants::AssumptionConstants, ΔHat::DeltaHat)
...
true_sys_constants(assump_consts::AssumptionConstants) = begin
    DeltaHat = DeltaHat_expr(assump_consts)
    Delta_circledcirc= Delta_circledcirc_expr(assump_consts, DeltaHat)
    Delta_odot= Delta_odot_expr(assump_consts, DeltaHat)
    Delta_otimes = Delta_otimes_expr(assump_consts, DeltaHat)
    Delta_ostar = Delta_ostar_expr(assump_consts, DeltaHat)
